...
parser.add_argument('--json_file', type=str, default="args_L.json",
...
parser.add_argument('--verbose', type=int, choices=list(range(2)), default=1,
                    help='Verbose during learning rounds.')
parser.add_argument('--plot', type=int, choices=list(range(2)), default=1,
                    help='Plots figure of sample complexity, performance, distance to "true" theta and bandit instance.')
parser.add_argument('--plot_rounds', type=int, choices=list(range(3)), default=0, 
...
parser.add_argument('--n_das', type=int,
                    help='Number of challengers to be sampled at each round (0 = only best guess, 1 = best guess + challenger, 2 = best guess + 2 challengers, ...).')
...
if (args.data not in ["linear", "classic", "logistic", "ofdm"]):
...
if (args.mode in ["small_test", "recommendation", "generate_latex"]):
	...
	path_to_plots += "_ndas="+str(args.n_das)
	path_to_plots += "/"
	...
	with open(path_to_plots+"last_command.txt", "w") as f:
		cmd = "python "+functools.reduce(lambda x,y : x+" "+y, sys.argv)
		f.write(cmd)
...
def select_bandit(X, df_di, oracle, data=args.data, theta=None, T_init=args.T_init, m=args.m, n_das=args.n_das, sigma=args.sigma, alpha=args.alpha, eta=args.eta, k1_diff=args.k1_diff, is_greedy=args.is_greedy, plot_step=args.plot_step, epsilon=args.epsilon, delta=args.delta, verbose=args.verbose, use_tracking=args.use_tracking, print_test=True):
	...
	assert args.beta
	assert S > 0
	args_problem = {"sigma": sigma, "grn_name": args.grn_name, "df_di": df_di, "path_to_grn": args.path_to_grn}
	...
	if (utils.is_of_type(T_init, "NoneType") and args.bandit == "TrueUniform"):
		...
		print(("Finetuning T_init in seq("+str(start)+", "+str(end)+", "+str(step)+") across "+str(n_simu)+" simulations "), end=' ')
		...
		T_init = list(range(start, end+step+1, step))[np.argmax(pes.T)]
		print(("T_init = "+str(T_init)))
		assert utils.is_of_type(T_init, "int")
		assert T_init > 0
	params+="_T_init="+str(T_init)
	args_ = {"beta": beta, "n_das": n_das, "plot_name": args.bandit+"_"+args.beta, "sigma": sigma, "alpha": alpha, "eta": eta, "k1_diff":k1_diff, "T_init": T_init, "is_greedy": is_greedy, "use_chernoff": args.use_chernoff, "use_tracking": use_tracking}
...
def print_names(bandit, min_list=5, ndec=5):
	...
	if (not isinstance(bandit.empirical_recommendation, type(None)) and not isinstance(names, type(None))):
		...
		output += str(list([round(x, ndec) for x in [bandit.empirical_means[i] for i in ids]]))+"\n"
...
## Meaning we run the bandit on the full dataset (for real-life data)
	...
	assert args.bandit
	assert args.n_simu
	bandit = select_bandit(X, df_di, oracle, args.data, n_das=args.n_das, m=m, theta=theta)
	...
	recommended = sorted(bandit.recommend())
	print(recommended)
	print((recommended[0]))
	print("* Oracle")
...
## Meaning we select/generate a smaller dataset
	...
	if (not os.path.exists(Xprime_filepath) or not os.path.exists(scores_filepath)):
		...
		test_idx = sample(list(range(K)), k)
	...
	if (args.plot):
		...
		utils.plot_instance(bandit)
	bandit.plot_results(n_simu=args.n_simu, show_plot=args.plot, plot_rounds=args.plot_rounds)
	plot_mat = np.zeros((2, bandit.t), dtype=int)
...
## along with a boxplot which compares the sample complexity across algorithms for the specified experiment
	...
	utils.build_boxplot_per_experiment_time(path_to_plots, args.data, K, N, m, args.problem, omega_str)
	#utils.build_boxplot_per_experiment_comparisons(path_to_plots, args.data, K, N, m, args.problem, omega_str)
else:
